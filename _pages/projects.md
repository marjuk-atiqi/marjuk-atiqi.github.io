---
permalink: /projects/
title: "Technical Projects & Implementations"
---

While my professional experience covers the management and execution of various initiatives, this section highlights the specific technical implementations, system architectures, and core modules I have delivered across major enterprise projects.

### 1. Enterprise ERP Deployments (Oracle R12)
**Clients:** Navana Group, The Metal (Pvt.) Limited, Karnafully Steel Mills Ltd.
* **Technical Focus:** Implemented Oracle Applications R12 Financial modules including Accounts Payable, Accounts Receivable, General Ledger, Cash Management, and Fixed Asset.
* **Key Contributions:** Conducted extensive AS-IS and TO-BE business process analysis, configured CRP (Conference Room Pilot) servers, and developed comprehensive functional documentation to map business needs to system architecture.

### 2. Jamuna Future Park ERP Integration
**Client:** Jamuna Group
* **Tech Stack:** Oracle Applications R12, Developer 10g R2 (Forms and Reports), BI Publisher, TOAD for Oracle, and Data Loader.
* **Key Contributions:** Deployed critical financial modules on high-performance Intel processor-based servers. Managed complex data loading processes and customized reporting tools using BI Publisher and Developer 10g.

### 3. Custom ERP Architecture
**Client:** Bangladesh Ordnance Factory (BOF)
* **Technical Focus:** Custom System Design & Data Integrity.
* **Key Contributions:** Architected a bespoke ERP solution by translating highly specialized operational workflows into scalable technical requirements, ensuring stringent data security and system reliability for defense-level operations.

### 4. MIS & BIS Modernization
**Client:** Palli Karma-Sahayak Foundation (PKSF)
* **Technical Focus:** System Optimization & Workflow Redesign.
* **Key Contributions:** Completely redesigned the architecture of legacy Management Information System (MIS) and Business Information System (BIS) modules. Optimized database performance to ensure real-time, large-scale data accessibility for stakeholders.

### 5. Hospital Management Modules
**Client:** Ibrahim Cardiac Hospital & Research Institute
* **Tech Stack:** Oracle 10g, Oracle Forms 10g, Oracle Reports 10g, PL/SQL.
* **Key Contributions:** Engineered comprehensive HRM and Inventory Management solutions from scratch. Wrote complex database triggers, stored procedures, and PL/SQL packages to seamlessly centralize hospital records and optimize medical stock control.
